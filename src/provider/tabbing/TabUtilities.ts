import {TabBlob, TabIdentifier, TabPackage, TabWindowOptions, ApplicationUIConfig} from '../../client/types';
...
export function isUrlTheSame(uuid: string, otherUuid: string): boolean {
    const appConfig: ApplicationUIConfig | undefined = TabService.INSTANCE.getAppUIConfig(uuid);
    const otherAppConfig: ApplicationUIConfig | undefined = TabService.INSTANCE.getAppUIConfig(otherUuid);

    if (appConfig && otherAppConfig) {
